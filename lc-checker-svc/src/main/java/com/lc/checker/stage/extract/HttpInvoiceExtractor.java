 ...
 * code drives retry policy and fallback decisions in {@link InvoiceExtractionOrchestrator}.
 */
    ...
    public ExtractionResult extract(byte[] pdfBytes, String filename) {
        ...
        for (int attempt = 0; attempt <= retries; attempt++) {
            try {
                ...
                // Python parsers (Docling / MinerU) contain NO internal LLM calls per the
                // v2 architecture — raw_markdown/text and heuristic fields only.
                return ExtractionResult.of(mapper.toDocument(dto));
