...
async def extract(
    ...
    # Outer span: wraps both PDF parse and LLM field-extract so reviewers
    # see two siblings under one Python-side parent in Langfuse. The span
    ...
    with track_span(
        ...
        # PDF parse phase — MinerU's library work (layout + OCR + markdown).
        # 25 s service budget enforced cooperatively via wait_for.
        with track_span(
            "mineru.pdf-parse",
            attributes={
                "pdf.input_mode": mode,
        ...
        # nests under `mineru.extract` thanks to current-context parent
        # inheritance.
        try:
